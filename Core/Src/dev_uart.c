...
/* 串口设备实例 */
static uart_device_t s_uart_dev = {0};
...
{
		/* 配置串口1收发fifo */
		fifo_register(&s_uart_dev.tx_fifo, &s_uart1_tx_buf[0], 
                      sizeof(s_uart1_tx_buf), fifo_lock, fifo_unlock);
		fifo_register(&s_uart_dev.rx_fifo, &s_uart1_rx_buf[0], 
                      sizeof(s_uart1_rx_buf), fifo_lock, fifo_unlock);
		
		/* 配置串口1 DMA收发buf */
		s_uart_dev.dmarx_buf = &s_uart1_dmarx_buf[0];
		s_uart_dev.dmarx_buf_size = sizeof(s_uart1_dmarx_buf);
		s_uart_dev.dmatx_buf = &s_uart1_dmatx_buf[0];
		s_uart_dev.dmatx_buf_size = sizeof(s_uart1_dmatx_buf);
        HAL_UARTEx_ReceiveToIdle_DMA(&huart1, s_uart_dev.dmarx_buf, sizeof(s_uart1_dmarx_buf));
		// bsp_uart1_dmarx_config(s_uart_dev.dmarx_buf, 
...
{
	return fifo_write(&s_uart_dev.tx_fifo, buf, size);
}
...
{
	return fifo_read(&s_uart_dev.rx_fifo, buf, size);
}
...
void uart_dmarx_done_isr(uint8_t uart_id)
{
	...
	recv_size = s_uart_dev.dmarx_buf_size - s_uart_dev.last_dmarx_size;
    s_UartTxRxCount[uart_id*2+1] += recv_size;
	fifo_write(&s_uart_dev.rx_fifo, 
				   (const uint8_t *)&(s_uart_dev.dmarx_buf[s_uart_dev.last_dmarx_size]), recv_size);

	s_uart_dev.last_dmarx_size = 0;
}
 ...
 * @retval 
 */
void uart_dmarx_half_done_isr(uint8_t uart_id, uint16_t buf_remain_size)
{
	...
	if(uart_id == 0)
	{
	  	recv_total_size = buf_remain_size;
	}
	recv_size = recv_total_size - s_uart_dev.last_dmarx_size;
	s_UartTxRxCount[uart_id*2+1] += recv_size;
    
	fifo_write(&s_uart_dev.rx_fifo, 
				   (const uint8_t *)&(s_uart_dev.dmarx_buf[s_uart_dev.last_dmarx_size]), recv_size);
	s_uart_dev.last_dmarx_size = recv_total_size;
}
 ...
 * @retval 
 */
void uart_dmarx_idle_isr(uint8_t uart_id, uint16_t buf_remain_size)
{
  	uint16_t recv_total_size;
  	uint16_t recv_size;
...
void uart_poll_dma_tx(uint8_t uart_id)
{
	...
	if (0x01 == s_uart_dev.status)
    {
        return;
    }
	size = fifo_read(&s_uart_dev.tx_fifo, s_uart_dev.dmatx_buf,
					 s_uart_dev.dmatx_buf_size);
	if (size != 0)
	{	
        s_UartTxRxCount[uart_id*2+0] += size;
	  	if (uart_id == 0)
		{
            s_uart_dev.status = 0x01;	/* DMA发送状态 */
		  	bsp_uart1_dmatx_config(s_uart_dev.dmatx_buf, size);
		}
		else if (uart_id == 1)
		{
            s_uart_dev.status = 0x01;	/* DMA发送状态,必须在使能DMA传输前置位，否则有可能DMA已经传输并进入中断 */
			bsp_uart2_dmatx_config(s_uart_dev.dmatx_buf, size);
		}
	}
}